...
const ApplicationsManagement = () => {
    ...
    const acceptForm = async (id) => {
        try {
            const response = await axios.put(`f/update/${id}`)
            getApplicationForms()
        } catch {
            toast.error("Wystąpił bład update")
        }
    }

    const declineForm = async (id) => {
        try {
            const response = await axios.put(`f/updatedecline/${id}`)
            getApplicationForms()
        } catch {
            toast.error("Wystąpił błąd update")
        }
    }
    ...
    return (
        <>
            ...
            <div class="modal-body text-center pb-5">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <br /> <br />
                            <h2 class="portfolio-modal-title text-secondary text-uppercase mb-0">Zarządzanie zgłoszeniami</h2>
                                            ...
                                            formsList.map((row) => {
                                                return (
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-xl btn-outline-dark" onClick={
                                                                () => {
                                                                    acceptForm(row.id)
                                                                }
                                                            }>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body>
                    Czy na pewno chcesz usunąć zgłoszenie? To operacja bez powrotu!
                </Modal.Body>
                <Modal.Footer>
